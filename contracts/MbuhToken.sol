  ...
  
{
  using SafeMath for uint256;
    ...
    uint256 private _farmingRewardDist; // farming rewards distribution (locked)
    uint256 private _devDist; // marketing & development distribution (unlocked)
    ...
    mapping(address => address) _mapAssetToPair; // Asset --> MBT-Asset Pair
    ...
    modifier onlyOperator() {
        require(
            ...
            "MBT: caller is not the operator"
        );
        _;
    }
    ...
    function initialize(
        ...
        __Ownable_init();
        __MbuhToken_init_unchained(_dev);
    }
    ...
    function __MbuhToken_init_unchained(
        //address smartComp,
        address dev
        //address airdrop
    ...
    function _approve(
        ...
        require(owner != address(0), "MBT: approve from the zero address");
        require(spender != address(0), "MBT: approve to the zero address");
        ...
        if (_msgSender() != recipient || !_excludedFromFee[recipient])
            _approve(
                ...
                _allowances[sender][_msgSender()].sub(
                    amount,
                    "MBT: transfer amount exceeds allowance"
                )
            );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "MBT: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function _transferFrom(
        ...
        require(sender != address(0), "MBT: transfer from the zero address");
        require(recipient != address(0), "MBT: transfer to the zero address");
        require(
            _balances[sender] >= amount,
            "MBT: balance of sender is too small."
        );
    ...
    function distributeSellTax(address sender, uint256 amount) internal {
        ...
        if (!_isLockedAchievementTax) {
            uint256 achievementAmount = amount.mul(_sellAchievementFee).div(
                100
            );
    ...
    /**
     * @dev Distribute tax to golden Tree pool as MBT and notify
     */
        ...
        onlyOperator
    {
        require(
            ...
            "MBT: already excluded or included"
        );
        _excludedFromFee[account] = excluded;
    ...
    function setSellFee(uint256 sellTaxFee) external onlyOperator {
        require(sellTaxFee < 100, "MBT: sellTaxFee exceeds maximum value");
    ...
    function setBuyFee(uint256 buyTaxFee) external onlyOperator {
        require(buyTaxFee < 100, "MBT: buyTaxFee exceeds maximum value");
    ...
    function setTransferFee(uint256 transferTaxFee) external onlyOperator {
        require(
            transferTaxFee < 100,
            "MBT: transferTaxFee exceeds maximum value"
        );
    ...
    function setGoldenTreeAddress(address _address) external onlyOperator {
        require(_address != address(0x0), "MBT: not allowed zero address");
